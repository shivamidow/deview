...
void EmitAssemblyHelper::CreatePasses(legacy::PassManager &MPM,
  ...
  if (CodeGenOpts.enable_web_api_marking) {
    if (PMBuilder.OptLevel < 1) {
      FPM.add(createSROAPass());
      MPM.add(createPromoteMemoryToRegisterPass());
    }
    MPM.add(createEnableWebAPIMarkingPass());
  }
  if (CodeGenOpts.enable_web_api_profiling) {
    if (PMBuilder.OptLevel < 1) {
      FPM.add(createSROAPass());
      MPM.add(createPromoteMemoryToRegisterPass());
    }
    MPM.add(createEnableWebAPIProfilingPass());
  }
